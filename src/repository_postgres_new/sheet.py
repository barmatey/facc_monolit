...
class SindexRepo(BaseRepo):
    ...
    async def _retrieve_scroll_size_with_session(self, session: AsyncSession,
    ...
    async def _update_scroll_pos_with_session(self, session: AsyncSession, sheet_id: core_types.Id_) -> None:
        # Get data
        sindexes = await super()._retrieve_bulk_as_dataframe_with_session(session,
...
class CellRepo(BaseRepo):
    ...
    async def update(self, sheet_id: core_types.Id_, data: entities.UpdateCell) -> None:
        async with db.get_async_session() as session:
            ...
            await super()._update_with_session(session, filter_by=filter_, data=data)
            await session.commit()
...
class SheetRepo(BaseRepo):
    ...
    async def _create_with_session(self, session: AsyncSession, data: entities.SheetCreate) -> core_types.Id_:
        sheet: Sheet = await super()._create_with_session(session, {})
        sheet_id = sheet.id
    ...
    async def retrieve_as_sheet(self, data: entities.SheetRetrieve) -> entities.Sheet:
        async with db.get_async_session() as session:
            return await self._retrieve_as_sheet_with_session(session, data)

    async def _retrieve_as_sheet_with_session(self, session: AsyncSession,
    ...
    async def retrieve_as_dataframe(self, id_: core_types.Id_) -> pd.DataFrame:
        async with db.get_async_session() as session:
            cells = await self.cell_repo()._retrieve_bulk_as_dataframe_with_session(session, {"sheet_id": id_})
            rows = await self.row_repo()._retrieve_bulk_as_dataframe_with_session(session, {"sheet_id": id_})
            cols = await self.col_repo()._retrieve_bulk_as_dataframe_with_session(session, {"sheet_id": id_})
    ...
    async def retrieve_scroll_size(self, id_: core_types.Id_) -> entities.ScrollSize:
        async with db.get_async_session() as session:
            row: SindexScrollSize = await self.row_repo()._retrieve_scroll_size_with_session(session, sheet_id=id_)
            col: SindexScrollSize = await self.col_repo()._retrieve_scroll_size_with_session(session, sheet_id=id_)
