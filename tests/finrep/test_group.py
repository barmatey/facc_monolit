...
def test_create_simple_balance_group(wire_df):
    wire = Wire(wire_df)
    real_df: pd.DataFrame = BalanceGroup().create_group_df(wire, ccols=['sender']).get_group_df()
...
def test_update_simple_balance_group(wire_df):
    wire = Wire(wire_df)
    old_group_df = BalanceGroup().create_group_df(wire, ccols=['sender']).get_group_df().head(5)
